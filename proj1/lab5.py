...
WEIGHT = np.array([[500, -800, 100, 50, 50, 100, -800, 500],
                   [-800, -1200, 50, 10, 10, 50, -1200, -800],
                   [100, 50, 200, 5, 5, 200, 50, 100],
                   [50, 10, 5, 5, 5, 5, 10, 50],
                   [50, 10, 5, 5, 5, 5, 10, 50],
                   [100, 50, 200, 5, 5, 200, 50, 100],
                   [-800, -1200, 50, 10, 10, 50, -1200, -800],
                   [500, -800, 100, 50, 50, 100, -800, 500]])
INF = float("inf")
...
class AI(object):
    ...
    def go(self, chessboard):
        ...
        if empty < 8:
            ...
            for e in self.candidate_list:
                value = self.alpha_beta_search(self.move(chessboard, e, self.color), 4, 1, (alpha, beta))
                print(e, value)
                if value[2] > alpha:
                    alpha = value[2]
                    temp_list.append(e)
    ...
    def evaluate(self, chessboard, current_layer, empty):
        ...
        if empty > 40:
            m_weight, b_weight, s_weight = 800, 1, 2500
        elif empty > 15:
            m_weight, b_weight, s_weight = 1500, 1, 600
        else:
            m_weight, b_weight, s_weight = 1000, 1, 600
    ...
    def get_mobility(self, chessboard, color):
        a, b = self.get_avail(chessboard, color), self.get_avail(chessboard, color * -1)
        if len(a) + len(b) == 0:
            return self.final_evaluate(chessboard) * self.color
        value = (len(a) - len(b)) / (len(a) + len(b)) + 1  # 0-1
        for e in a:
    ...
    def get_board_weight(self, chessboard):
        ...
        if self.color_num(chessboard[0, 2:6], self.color) == 4:
            weight[0][1], weight[0][6] = 200, 200
        if self.color_num(chessboard[7, 2:6], self.color) == 4:
            weight[7][1], weight[7][6] = 200, 200
        if self.color_num(chessboard[2:6, 0], self.color) == 4:
            weight[1][0], weight[6][0] = 200, 200
        if self.color_num(chessboard[2:6, 7], self.color) == 4:
            weight[1][7], weight[6][7] = 200, 200
        if np.sum(chessboard[0, 2:6]) == -4 * self.color:
            value -= 300 * self.color
        if np.sum(chessboard[7, 2:6]) == -4 * self.color:
            value -= 300 * self.color
        if np.sum(chessboard[2:6, 0]) == -4 * self.color:
            value -= 300 * self.color
        if np.sum(chessboard[2:6, 7]) == -4 * self.color:
            value -= 300 * self.color
        return np.sum(chessboard * weight) + value
